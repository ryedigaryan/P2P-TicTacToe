...
public class AppFlow implements AppStateEventHandler, Runnable {
     ...
     * If the {@link AppStateEvent#shouldStopPreviousAppFlowItem()} returns true, then old flow item is stopped, instead of pausing.
     ...
     */
    public <E extends AppStateEvent> void registerAppFlowItemChangeRule(Integer oldFlowItemId, E event, Supplier<AppState> newFlowItem) {
    ...
    @Override
    public void handleAppFlowItemEvent(AppState eventSource, AppStateEvent event) {
        ...
        assert currentAppState.getAppStateEventHandler() == this : "Event handler of all AppFlowItems should be AppFlow.";
